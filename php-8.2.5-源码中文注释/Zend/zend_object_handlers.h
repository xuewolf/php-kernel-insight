...
// 把对象转换成其他类型，只读对象 和 返回值必须指向到不同 zval
...
// ing4, 取得方法的根类：如果有原型（->common.prototype）返回原型的 作用域。否则使用本身的作用域
...
// get_properties_for 的默认行为。在自定义 get_properties_for 实现中，用作回调。
...
// 调用 get_properties_for 处理器或使用默认行为。给用户调用 get_properties_for API。
...
// ing4, 如果是执行时中的弹跳函数？清空当前弹跳函数名：否则释放此函数
...
// ing4, 如果不 （都是对象 并且 有同样的 compare() 函数）调用默认比较方法。
// 除非想要明确地支持对象和非对象的比较，否则都应该调用这个方法
...
#define ZEND_COMPARE_OBJECTS_FALLBACK(op1, op2) \
	/* 运算对象1不是 对象 或 运算对象2不是 对象 或两个对象的compare方法不同 */ \
