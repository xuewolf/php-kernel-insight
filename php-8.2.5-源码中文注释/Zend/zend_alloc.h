...
// windows 里多次 malloc 分配的内存不是连续的，无法越界计算位置。

// 重要的东西都在 zend_alloc.c里
...
// ZEND_MM_ALIGNMENT 一定要在配置文件里定义好！，windows默认是 (size_t)8，0000 1000，（测试过）
...
// ing4, 对齐到8， ( size + 7 ) & -8。 +7然后清除最后3个bit （验证过）
#define ZEND_MM_ALIGNED_SIZE(size)	(((size) + ZEND_MM_ALIGNMENT - 1) & ZEND_MM_ALIGNMENT_MASK)

// ing4, size + n个1 & n个0, 向后对齐 alignment
...
// zend的内存管理比原生方法更加持久稳定
/* Selective persistent/non persistent allocation macros */
// persistent 为 1时，调用 __zend_malloc（原生方法），0时调用 zend方法 emalloc -> _emalloc
...
// persistent 为1时调用原生方法，0时调用 efree，zend的方法。为什么和上面相反呢
...
// 多个 ZEND_FILE_LINE_RELAY_CC 参数 # define ZEND_FILE_LINE_RELAY_C __zend_filename, __zend_lineno
...
/*
   下面的例子展示了如何在自己的存储中用 zend_mm_heap API 
