...
// 如果有 c++？ （这个东西 测试未通过，看着也不符合基本语法规则）
#ifdef __cplusplus
#define BEGIN_EXTERN_C() extern "C" {
#define END_EXTERN_C() }
// 如果没有 c++，这个 BEGIN_EXTERN_C 没作用
#else
...
// 如果是windows 操作系统  并且没有定义 __clang__
...
// 如果是windows 并且没定义 __clang__（经测试，没有这个东西）
#if defined(ZEND_WIN32) && !defined(__clang__)
// （经测试 windows 下 __assume 没什么反应）
...
// 如果要用这个宏，要自己保证所有的 switch 值 都已经被 case语句 覆盖
...
// 不是调试模式 并且 (有 HAVE_ALLOCA 或 __GNUC__ )
...
// clear, offsetof 的别名，取得元素相对于结构体的偏移量
#ifndef XtOffsetOf
...
// ing4, 重新定义 MIX,MAX 两个常用宏
#undef MIN
#undef MAX
#define MAX(a, b)  (((a)>(b))?(a):(b))
#define MIN(a, b)  (((a)<(b))?(a):(b))

// clear , 验证某一个位是否是1。 考虑到兼容性的程序可真难写哇！
// bits 里面每个块8 bytes,64 bits
#define ZEND_BIT_TEST(bits, bit) \
	(((bits)[(bit) / (sizeof((bits)[0])*8)] >> ((bit) & (sizeof((bits)[0])*8-1))) & 1)
// (((bits)[(bit) / (8*8)] >> ((bit) & (8*8-1))) & 1)
// (((bits)[(bit) / (64)] >> ((bit) & (63))) & 1)
// (((bits)[块号] >> ( (bit)%64 )) & 1)
// (((bits)[块号] >>  bit在块中的位置 ) & 1)
// 移动后，这个bit就到了右侧第一位 & 1
// 得到这个bit 是否是 1 
...
// 返回本身和 长度-1（长度），最后一个 \0 不算
#define ZEND_STRL(str)		(str), (sizeof(str)-1)

// 返回本身和 占内存大小，最后一个 \0 也算进来
...
// xor , ^ 操作。两个结果必须不同 00=0, 01=1, 10=0, 11=0。
